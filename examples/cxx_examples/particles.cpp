...
static void draw_particles(glfw::Window* window, double t, float dt)
{
...
static void draw_scene(glfw::Window* window, double t)
{
...
{
    glfw::Window* window = reinterpret_cast<glfw::Window*>(arg);
...
int main(int argc, char** argv)
{
    ...
    window = glfw::CreateWindow(width, height, "Particle Engine", monitor, NULL);
    if (!window)
    {
    ...
    if (monitor)
        glfw::SetInputMode(window, GLFW_CURSOR, GLFW_CURSOR_DISABLED);
    ...
    if (glfw::ExtensionSupported("GL_EXT_separate_specular_color"))
    {
    ...
    while (!glfw::WindowShouldClose(window))
    {
        draw_scene(window, glfw::GetTime());
