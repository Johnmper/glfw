...
int main(int argc, char** argv)
{
    ...
    windows[0] = glfw::CreateWindow(400, 400, "First", NULL, NULL);
    if (!windows[0])
    {
        glfw::Terminate();
        exit(EXIT_FAILURE);
    ...
    glfw::SetKeyCallback(windows[0], key_callback);
    ...
    // All objects will be shared with the second context, created below
    {
        ...
        srand((unsigned int) glfw::GetTimerValue());
    ...
    windows[1] = glfw::CreateWindow(400, 400, "Second", NULL, windows[0]);
    if (!windows[1])
    {
        glfw::Terminate();
        exit(EXIT_FAILURE);
    ...
    // Place the second window to the right of the first
    {
        ...
        glfw::GetWindowSize(windows[0], &width, NULL);
        glfw::GetWindowFrameSize(windows[0], &left, NULL, &right, NULL);
        glfw::GetWindowPos(windows[0], &xpos, &ypos);

        glfw::SetWindowPos(windows[1], xpos + width + left + right, ypos);
    }

    glfw::SetKeyCallback(windows[1], key_callback);
           ...
           !glfw::WindowShouldClose(windows[1]))
    {
        ...
        for (i = 0;  i < 2;  i++)
        {
            ...
            glfw::GetFramebufferSize(windows[i], &width, &height);
