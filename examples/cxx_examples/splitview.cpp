...
static void framebufferSizeFun(glfw::Window* window, int w, int h)
{
    width  = w;
    height = h > 0 ? h : 1;
    do_redraw = 1;
...
static void windowRefreshFun(glfw::Window* window)
{
    drawAllViews();
...
static void cursorPosFun(glfw::Window* window, double x, double y)
{
    int wnd_width, wnd_height, fb_width, fb_height;
    double scale;

    glfw::GetWindowSize(window, &wnd_width, &wnd_height);
...
static void mouseButtonFun(glfw::Window* window, int button, int action, int mods)
{
...
int main(void)
{
    ...
    // Open OpenGL window
    window = glfw::CreateWindow(500, 500, "Split view demo", NULL, NULL);
    if (!window)
    {
    ...
    glfw::SetWindowRefreshCallback(window, windowRefreshFun);
    glfw::SetCursorPosCallback(window, cursorPosFun);
    glfw::SetMouseButtonCallback(window, mouseButtonFun);
