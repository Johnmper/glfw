...
void mouse_button_callback(glfw::Window* window, int button, int action, int mods)
{
    ...
    if (action == GLFW_PRESS)
    {
        ...
        glfw::GetCursorPos(window, &cursorX, &cursorY);
    }
    else
        glfw::SetInputMode(window, GLFW_CURSOR, GLFW_CURSOR_NORMAL);
}
...
{
    if (glfw::GetInputMode(window, GLFW_CURSOR) == GLFW_CURSOR_DISABLED)
    {
...
int main(int argc, char* argv[])
{
    ...
    window = glfw::CreateWindow(640, 480, "Wave Simulation", NULL, NULL);
    if (!window)
    {
        glfw::Terminate();
        exit(EXIT_FAILURE);
